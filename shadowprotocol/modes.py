...
class ModeA(BaseMode):
    ...
    def get_label(self) -> str:
        """Return the mode label identifier."""
        return "A"

    def validate_offset(self) -> bool:
        """Validate offset format & existence with deep pattern search"""
        ...
        try:
            # Phase 1: Deep disassembly around offset (wider range)
            success, _, error = self.r2.execute(f"s {self.offset}")
            if not success:
                self.log(f"[!] r2 seek error: {error}")
            
            # Extended disassembly: 100 instructions for deeper analysis
            success, disasm_main, error = self.r2.execute("pd 100")
            ...
            # Phase 2: Look for pattern variations
            patterns = [
                r'add\s+x\d+,\s*x\d+,\s*0x30',  # add x?,x?,0x30
                r'add\s+w\d+,\s*w\d+,\s*0x30',  # add w?,w?,0x30
                r'0x30',                          # Just the constant
                r'add.*0x30',                     # add with 0x30
            ]
            ...
            for pattern in patterns:
                if re.search(pattern, disasm_main, re.IGNORECASE):
                    pattern_found = True
                    matched_pattern = pattern
                    break
            
            if pattern_found:
                self.log(f"[+] Pattern '{matched_pattern}' found at {self.offset}")
                self.r2.close()
                return True
            
            # Phase 3: Fallback - scan backward before offset
            self.log(f"[*] Pattern not at exact offset, scanning backward...")
            success, disasm_before, error = self.r2.execute("pd -50")
            
            if success:
                for pattern in patterns:
                    if re.search(pattern, disasm_before, re.IGNORECASE):
                        self.log(f"[+] Pattern '{pattern}' found BEFORE offset (nearby)")
                        self.r2.close()
                        return True
            
            # Phase 4: Scan forward after offset
            self.log(f"[*] Scanning forward after offset...")
            success, disasm_after, error = self.r2.execute("s {self.offset}+0x100; pd 50")
            
            if success:
                for pattern in patterns:
                    if re.search(pattern, disasm_after, re.IGNORECASE):
                        self.log(f"[+] Pattern '{pattern}' found AFTER offset (nearby)")
                        self.r2.close()
                        return True
            
            self.log("[W] Pattern not found in offset vicinity (±100 instructions)")
            self.log("[W] Validation failed - offset may be incorrect or pattern not present")
            self.r2.close()
            return False
    ...
    def patch(self) -> bool:
        """Apply patch via Radare2 with improved verification"""
        if not self.r2:
            self.log("[!] No binary loaded for Radare2")
            return False
        ...
        try:
            ...
            # Extended verification with deeper disassembly
            success, verify, error = self.r2.execute("pd 20")
            self.r2.close()
            ...
            # Check for patched instruction
            if "0x20" in verify or "add" in verify:
                self.log("[+] Patch applied and verified")
                return True

            self.log("[!] Patch verification failed - 0x20 not found in instruction")
            return False
        except Exception as e:
